package com.now.kitt.feature_note.presentation
...
import com.now.kitt.feature_note.presentation.notes.NotesState
...
class NotesViewModel @Inject constructor(
    ...
    fun onEvent(event: NotesEvent){
        when(event){
            is NotesEvent.Order -> {
                if(state.value.noteOrder::class == event.noteOrder::class && event.noteOrder.orderType==event.noteOrder.orderType ){
                    return
                }
            ...
            is NotesEvent.TogglesOrderSection -> {
